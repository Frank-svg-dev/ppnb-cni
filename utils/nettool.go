...
func CreateVxlanAndUp(name string, mtu int) (*netlink.Vxlan, error) {
	...
	if err != nil {
		fmt.Println("无法创建 vxlan 设备: ", name, "err: ", err.Error())
		return nil, err
	}

	l, err = netlink.LinkByName(name)
	if err != nil {
		fmt.Println("获取 vxlan 失败")
		return nil, err
	}

	vxlan, ok = l.(*netlink.Vxlan)
	if !ok {
		fmt.Println("找到了设备, 但是该设备不是 vxlan")
		return nil, fmt.Errorf("found the device %q but it's not a vxlan", name)
	}
	// 然后还要把这个 vxlan 给 up 起来
	if err = netlink.LinkSetUp(vxlan); err != nil {
		fmt.Println("启动 vxlan 失败, err: ", err.Error())
...
func CreateBridge(brName, gw string, mtu int) (*netlink.Bridge, error) {
	...
	if err != nil {
		fmt.Println("无法创建网桥: ", brName, "err: ", err.Error())
		return nil, err
	}
...
func SetVethToBridge(veth *netlink.Veth, br *netlink.Bridge) error {
	...
	if err != nil {
		fmt.Println("把 veth 插到网桥上失败, err: ", err.Error())
...
func SetVethMaster(veth *netlink.Veth, br *netlink.Bridge) error {
	err := netlink.LinkSetMaster(veth, br)
	if err != nil {
		fmt.Println(fmt.Sprintf("把 veth %q 干到 master 上失败: %v", veth.Attrs().Name, err))
...
func SetDeviceMaster(device *netlink.Device, br *netlink.Bridge) error {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("把 veth %q 干到网桥上失败: %v", device.Attrs().Name, err))
...
func SetIptablesForToForwardAccept(link netlink.Link) error {
	...
	if err != nil {
		fmt.Println("这里 NewWithProtocol 失败, err: ", err.Error())
		return err
	}
	err = ipt.Append("filter", "FORWARD", "-i", link.Attrs().Name, "-j", "ACCEPT")
	if err != nil {
		fmt.Println("这里 ipt.Append 失败, err: ", err.Error())
		return err
	}
	return nil
}
